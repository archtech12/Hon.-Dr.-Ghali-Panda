...
const sampleAboutData = {
  ...
  content: `<p>Dr. Ghali Mustapha Tijjani was born on June 13, 1978, in Albasu Phanda, Kano State. A dedicated advocate for community development and participatory leadership, Dr. Ghali entered national public service in 2023. He currently represents the Gaya, Ajingi, and Albasu Federal Constituency in the House of Representatives on the platform of the All Progressives Congress (APC).</p>
  ...
  imageUrl: "/assets/images/gallery/0-Portraits-Official/portrait-1.jpg",
  quickFacts: [
    ...
    { icon: 'cake', label: 'Born', value: 'June 13, 1978' },
...
const seed = async () => {
  try {
    ...
    if (existing) {
      console.log('Updating existing About data...');
      await About.findByIdAndUpdate(existing._id, sampleAboutData);
    } else {
      console.log('Creating new About data...');
      await About.create(sampleAboutData);
    }

    console.log('About data seeded/updated successfully');
