    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-2">
            {(['square', 'portrait', 'story', 'landscape'] as AspectRatio[]).map(ratio => (
                <button
                    key={ratio}
                    onClick={() => onUpdate('aspectRatio', ratio)}
                    className={`p-2 rounded-lg border text-xs font-medium flex flex-col items-center justify-center gap-1 transition-all ${
                        data.aspectRatio === ratio
                        ? 'border-green-500 bg-green-50 text-green-700 ring-1 ring-green-500'
                        : 'border-gray-200 hover:border-gray-300 text-gray-600 bg-white'
                    }`}
                >
                    <span className={`block bg-current opacity-20 ${
                        ratio === 'square' ? 'w-4 h-4' : 
                        ratio === 'portrait' ? 'w-3 h-4' :
                        ratio === 'story' ? 'w-3 h-5' : 'w-5 h-3'
                    }`}></span>
                ...
                {['green', 'red', 'blue', 'gold'].map(color => (
                    <button
                        ...
                        className={`w-8 h-8 rounded-full border-2 transition-transform ${
                            data.colorTheme === color ? 'bg-white scale-110 ring-2 ring-offset-2 ring-gray-400' : 'border-transparent'
                        }`}
